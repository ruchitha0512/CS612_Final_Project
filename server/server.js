...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    const query = 'INSERT INTO users (name, email, password, handle, bio, avatar) VALUES (?, ?, ?, ?, ?, ?)';
    const values = [name, email, hashedPassword, handle, bio || null, avatar || DEFAULT_AVATAR];
...
app.delete('/api/comments/:id', auth, (req, res) => {
  ...
  app.get('/api/search/users', auth, (req, res) => {
    ...
    db.query(query, [searchTerm, searchTerm], (err, results) => {
      ...
      res.json(results); // Send matching users as the response
    });
  });
